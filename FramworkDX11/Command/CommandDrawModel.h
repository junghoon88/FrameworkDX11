...
#if 0
class CommandMeshSelect : public Command
{
public:
	CommandMeshSelect(Model::BinModel* selectedModel, Model::BinModel* pModel)
		: selectedModel(selectedModel), oldModel(selectedModel), newModel(pModel)
	{
	}
	~CommandMeshSelect()
	{
...
private:
	Model::BinModel* selectedModel;
	Model::BinModel* oldModel;
	Model::BinModel* newModel;
...
{
public:
	CommandModelLoad(vector<Model::BinModel*>& models, Model::BinModel* addModel)
		: models(models), loaded(false)
	{
		loadModels.push_back(addModel);
	}
	CommandModelLoad(vector<Model::BinModel*>& models, vector<Model::BinModel*> addModels)
	...
	~CommandModelLoad()
	{
		if (loaded == false)
		{
			for (Model::BinModel* model : loadModels)
				SAFE_DELETE(model);
		}
	}
...
private:
	...
	vector<Model::BinModel*> loadModels;
	bool loaded;
};
...
{
public:
	CommandModelDelete(vector<Model::BinModel*>& models, Model::BinModel* deleteModel)
		: models(models), deleted(false)
	{
		deleteModels.push_back(deleteModel);
	}
	CommandModelDelete(vector<Model::BinModel*>& models, vector<Model::BinModel*> deleteModels)
	...
	~CommandModelDelete()
	{
		if (deleted == true)
		{
			for(Model::BinModel* model : deleteModels)
				SAFE_DELETE(model);
		}
	}
	...
	virtual void Execute(void) override
	{
		...
		{
			for (UINT j = 0; j < deleteModels.size(); j++)
			{
				if (models[i] == deleteModels[j])
				{
					//indexs[j] = i;
...
private:
	vector<Model::BinModel*>& models;
	vector<Model::BinModel*>  deleteModels;
	bool deleted;
	//vector<int> indexs;
};
...
{
public:
	CommandModelCopy(vector<Model::BinModel*>& models, vector<Model::BinModel*> targetModels)
		: models(models), copied(false)
	{
		...
		{
			copyModels[i] = new Model::BinModel(*targetModels[i]);
		}
	}
	~CommandModelCopy()
	{
		if (copied == false)
		{
			for (Model::BinModel* model : copyModels)
				SAFE_DELETE(model);
		}
	}
...
private:
	vector<Model::BinModel*>& models;
	vector<Model::BinModel*>  copyModels;
	bool copied;
};
...
public:
	//모델들을 한곳으로 이동할 때
	CommandModelTransform(Model::BinModel* model, TRANSFORM transform)
	{
		models.push_back(model);
	...
	CommandModelTransform(vector<Model::BinModel*> models, vector<TRANSFORM> oldTransforms)
